...
router.get('/realTimeDispatchRider', util.isLoggedin, function( req, res, next) {
  mysqlConnect('delivery', 'realTimeDispatchRider', req.query, function (error, results) {
    if (error) {
      res.status(500).json(util.successFalse("SQL Error"));
    }
    var string = JSON.stringify(results);
...
router.post('/realTimeDispatch', util.isLoggedin, function( req, res, next) {
  mysqlConnect('delivery', 'validateDelivery', req.body, function (error, results) {
    if (error) {
      res.status(500).json(util.successFalse("SQL Error"));
    }
    var string = JSON.stringify(results);
    ...
    if( json[0].dlvryCnt == 1 ) {
      mysqlConnect('delivery', 'realTimeDispatch', req.body, function (error, results) {
        if (error) {
          res.status(500).json(util.successFalse("SQL Error"));
        }
        var string = JSON.stringify(results);
...
router.post('/realTimeCancelDelivery', util.isLoggedin, [
  ...
  mysqlConnect('delivery', 'validateDelivery', req.body, function (error, results) {
    ...
    var json =  JSON.parse(string);
    if( json[0].dlvryCnt == 1 ) {
      mysqlConnect('delivery', 'realTimeCancelDelivery', req.body, function (error, results) {
        if (error) {
          res.status(500).json(util.successFalse("SQL Error"));
        }
        var string = JSON.stringify(results);
...
// 배달 등록
router.post('/registerDelivery', util.isLoggedin, [
  ...
  var errors = validationResult(req);
  if( !errors.isEmpty() ) return res.status(400).json(util.successFalse(errors));
  mysqlConnect('delivery', 'registDeliveryKey', req.body, function (error, results) {
    if (error) {
      res.status(500).json(util.successFalse("SQL Error"));
    }
    var string = JSON.stringify(results);
    ...
    mysqlConnect('delivery', 'registDelivery', req.body, function (error, results) {
      if (error) {
        res.status(500).json(util.successFalse("SQL Error"));
      }
      var string = JSON.stringify(results);
...
// 배달 고객 정보 수정
router.post('/modifyDelivery', util.isLoggedin, [
  check('dlvryNo','주문 번호는 필수 입력 입니다. 상점ID(5) + "O" + 날짜(8) + 일련번호 형식으로 입력해 주세요.(ex : S0001O1900123100001)').exists().bail().notEmpty().bail().isLength({ min: 19, max: 19 }),
  ...
  check('dlvryCusDetlAdres','고객 상세 주소가 입력 되지 않았습니다.').optional().notEmpty(),
